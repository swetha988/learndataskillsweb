const QUIZ_BANK = {
  ...
  // ML Advanced
  'ml-a-1': {
    title: 'Check your understanding: Deep Learning with PyTorch',
    intro: 'Test your knowledge of tensors, autograd, and the PyTorch training loop.',
    questions: [
      { id: 'q1', prompt: 'What does calling loss.backward() do in PyTorch?', options: ['Prints the loss value', 'Computes gradients of the loss with respect to all parameters via backpropagation through the computation graph', 'Updates the model weights', 'Resets the model to its initial state'], answerIndex: 1, explanation: 'backward() traverses the computation graph in reverse, computing ∂loss/∂w for every parameter with requires_grad=True. The gradients are stored in param.grad — optimizer.step() then uses them.' },
      { id: 'q2', prompt: 'Why must you call optimizer.zero_grad() before loss.backward() each iteration?', options: ['To reset the loss to zero', 'PyTorch accumulates gradients by default — not zeroing them adds gradients from previous iterations, corrupting the update', 'To free GPU memory', 'To reinitialise the model weights'], answerIndex: 1, explanation: 'Gradient accumulation is a feature (useful for large-batch training), not a bug. But for standard training, you must zero gradients each step or they accumulate across batches.' },
      { id: 'q3', prompt: 'model.eval() vs model.train() matters because:', options: ['eval() is faster', 'Dropout and BatchNorm behave differently — Dropout is disabled in eval, BatchNorm uses running stats instead of batch stats', 'eval() prevents gradient computation', 'train() uses more GPU memory'], answerIndex: 1, explanation: 'Dropout randomly zeros neurons during training for regularisation — disabling it at test time gives consistent predictions. BatchNorm uses batch statistics during training but exponential moving average at eval.' },
      { id: 'q4', prompt: 'A learning rate scheduler (StepLR, CosineAnnealingLR) is used to:', options: ['Increase model depth over time', 'Reduce the learning rate during training — large lr early for fast learning, smaller lr later for fine-grained convergence', 'Switch between optimisers', 'Adjust batch size dynamically'], answerIndex: 1, explanation: 'Starting with a high lr and decaying it is standard practice. Large lr early = fast progress. Small lr later = settle into a sharp minimum without overshooting.' },
      { id: 'q5', prompt: 'nn.Sequential in PyTorch allows you to:', options: ['Run layers in parallel', 'Stack layers in order — forward() calls each layer in sequence automatically', 'Share weights between layers', 'Only use ReLU as the activation'], answerIndex: 1, explanation: 'nn.Sequential defines a chain of layers. Calling model(x) passes x through each layer in sequence. Cleaner than manually calling each layer in forward().' }
    ]
  },
  'ml-a-2': {
    title: 'Check your understanding: Convolutional Neural Networks',
    intro: 'Test your knowledge of convolution, pooling, and transfer learning.',
    questions: [
      { id: 'q1', prompt: 'Why does a CNN use convolution instead of fully connected layers for images?', options: ['Convolution is always faster', 'Convolution exploits spatial locality — the same filter detects a pattern anywhere in the image, with far fewer parameters than a fully connected layer', 'Fully connected layers don\'t work on pixel values', 'Convolution automatically labels images'], answerIndex: 1, explanation: 'A 224×224 image × 3 channels = 150,528 inputs. A single FC layer to 1,000 neurons = 150M parameters. A 3×3 conv filter has only 27 parameters and detects the same pattern everywhere — parameter sharing.' },
      { id: 'q2', prompt: 'MaxPool2d(2) reduces a 14×14 feature map to:', options: ['12×12', '7×7', '28×28', '14×7'], answerIndex: 1, explanation: 'MaxPool2d(kernel_size=2, stride=2) takes the maximum in each 2×2 region. 14/2 = 7. Each spatial dimension is halved — reducing computation and adding translation invariance.' },
      { id: 'q3', prompt: 'BatchNorm2d after a Conv2d layer:', options: ['Reduces the number of filters', 'Normalises activations within the mini-batch — stabilises training, allows higher learning rates, reduces sensitivity to weight initialisation', 'Applies dropout to convolutional filters', 'Converts feature maps to vectors'], answerIndex: 1, explanation: 'Batch normalisation normalises the output of each channel across the batch. It effectively acts as regularisation, reduces internal covariate shift, and allows training with larger learning rates.' },
      { id: 'q4', prompt: 'In transfer learning, you freeze base layers and only train the final classification head. This works because:', options: ['Frozen layers train faster', 'Early CNN layers learn universal features (edges, textures, shapes) transferable across domains — only the final classifier needs retraining for your specific classes', 'Frozen layers reduce GPU memory', 'The pretrained weights are already optimal for all tasks'], answerIndex: 1, explanation: 'ImageNet-pretrained CNNs learn hierarchical visual features: early layers = edges/colours, middle = textures/parts, late = object-specific. These features transfer well to most visual tasks.' },
      { id: 'q5', prompt: 'CosineAnnealingLR as a learning rate scheduler gradually:', options: ['Increases the learning rate each epoch', 'Decreases the learning rate following a cosine curve from lr_max to lr_min over T_max epochs', 'Halves the learning rate every 10 epochs', 'Sets the learning rate to 0 after epoch T_max'], answerIndex: 1, explanation: 'Cosine annealing smoothly decays the learning rate in a cosine curve, which often outperforms step decay. The smooth decay helps the model settle into flatter, more generalisable minima.' }
    ]
  },
  'ml-a-3': {
    title: 'Check your understanding: Transformers and Attention',
    intro: 'Test your knowledge of self-attention, transformers, and HuggingFace.',
    questions: [
      { id: 'q1', prompt: 'Self-attention allows each token to:', options: ['Attend only to adjacent tokens', 'Attend to every other token in the sequence simultaneously — capturing long-range dependencies in a single operation', 'Attend only to the previous token', 'Generate new tokens'], answerIndex: 1, explanation: 'Unlike RNNs which pass information step-by-step (losing long-range context), self-attention computes attention scores between every pair of tokens in the sequence in parallel.' },
      { id: 'q2', prompt: 'Scaling by √d_k in the attention formula (Q×Kᵀ / √d_k) prevents:', options: ['Gradient explosion during backprop', 'Extremely large dot products that push softmax into saturation regions where gradients vanish', 'Negative attention scores', 'Attention weights summing to more than 1'], answerIndex: 1, explanation: 'As d_k grows, dot products grow in magnitude. Large values cause softmax to become very "peaky" — nearly all attention on one token, near-zero gradients elsewhere. Dividing by √d_k maintains stable gradients.' },
      { id: 'q3', prompt: 'Fine-tuning a pretrained transformer (e.g., DistilBERT) for classification involves:', options: ['Training the model from scratch on your dataset', 'Loading pretrained weights, replacing the final layer for your number of classes, and training on your data — usually for just 3-5 epochs', 'Using only the tokeniser from the pretrained model', 'Retraining all layers with a very high learning rate'], answerIndex: 1, explanation: 'Fine-tuning transfers pretrained knowledge to your task. The base layers already understand language — you just teach the final layer(s) your specific classification. Much less data and compute than pretraining.' },
      { id: 'q4', prompt: 'HuggingFace pipeline("sentiment-analysis") allows:', options: ['Only analysing positive/negative labels', 'Zero-shot inference with a pretrained model in 2 lines of code — no training required', 'Training a sentiment model from scratch', 'Connecting to a paid API only'], answerIndex: 1, explanation: 'HuggingFace pipelines abstract tokenisation, model inference, and post-processing. They enable production-quality NLP with minimal code, using models already fine-tuned on standard tasks.' },
      { id: 'q5', prompt: 'Why choose DistilBERT over BERT-large for a production deployment?', options: ['DistilBERT is always more accurate', 'DistilBERT is 60% the size, 97% the accuracy, and 2× faster — meeting business accuracy requirements with much lower latency and cost', 'DistilBERT can handle longer sequences', 'BERT-large cannot be fine-tuned'], answerIndex: 1, explanation: 'Knowledge distillation trains a smaller model to mimic a larger one. DistilBERT retains 97% of BERT\'s accuracy at 60% the size. In production, latency and cost matter — always pick the smallest model that meets requirements.' }
    ]
  },
  'ml-a-4': {
    title: 'Check your understanding: MLOps',
    intro: 'Test your knowledge of experiment tracking, drift detection, and production ML.',
    questions: [
      { id: 'q1', prompt: 'Data drift in production ML means:', options: ['Training data was accidentally deleted', 'The distribution of incoming production data has changed from what the model was trained on — causing performance degradation', 'The model weights have changed', 'The API endpoint moved'], answerIndex: 1, explanation: 'Data drift = P(X) changes. A fraud model trained on 2022 data may miss 2024 fraud patterns. Concept drift = P(Y|X) changes — the relationship between features and labels shifts (e.g., post-pandemic behaviour).' },
      { id: 'q2', prompt: 'MLflow mlflow.log_metrics() during training records:', options: ['The model architecture', 'Numeric metrics (AUC, F1, loss) linked to the run — queryable and comparable across experiments in the MLflow UI', 'Training data statistics', 'Git commit hash'], answerIndex: 1, explanation: 'MLflow stores metrics per step/epoch in a queryable format. The UI shows training curves, parallel coordinate plots for hyperparameter comparison, and diff between runs — essential for systematic experimentation.' },
      { id: 'q3', prompt: 'The Kolmogorov-Smirnov (KS) test for drift detection compares:', options: ['Two model predictions', 'The full distributions of a feature in training vs production — p < 0.05 signals statistically significant drift', 'Sample means only', 'Variance between datasets'], answerIndex: 1, explanation: 'KS test measures the maximum difference between two cumulative distribution functions. It is non-parametric — works for any distribution. Small p-value = the two samples are unlikely from the same distribution.' },
      { id: 'q4', prompt: 'A GitHub Actions ML pipeline with scheduled weekly retrain achieves:', options: ['Faster model inference', 'Automated model freshness — the model is retrained on new data regularly, combating data drift without manual intervention', 'Larger training datasets automatically', 'Reduced cloud costs'], answerIndex: 1, explanation: 'CI/CD for ML: automate train → evaluate → threshold check → deploy. The pipeline fails and alerts if AUC drops below threshold, preventing a degraded model from reaching production.' },
      { id: 'q5', prompt: 'Why is a separate "target network" used in DQN (and relevant to MLOps stability)?', options: ['The target network provides faster inference', 'Updating Q-targets and Q-predictions from the same network creates moving targets that cause unstable training — a frozen target network provides stable training signals', 'The target network handles data preprocessing', 'Two networks double the model capacity'], answerIndex: 1, explanation: 'In RL (and analogously in production): if your learning target keeps shifting as you learn, training diverges. Separating the prediction network (updated every step) from the target network (updated every N steps) stabilises learning.' }
    ]
  },
  'ml-a-5': {
    title: 'Check your understanding: Advanced Ensemble Methods',
    intro: 'Test your knowledge of XGBoost, LightGBM, and stacking.',
    questions: [
      { id: 'q1', prompt: 'Gradient boosting builds trees sequentially by:', options: ['Training each tree on a random subset of features', 'Each new tree fitting the residual errors (negative gradient of loss) of the ensemble so far — progressively correcting mistakes', 'Averaging all tree predictions equally', 'Growing the deepest tree first'], answerIndex: 1, explanation: 'Gradient boosting minimises loss function by adding trees in the direction of the negative gradient. Each tree corrects where the current ensemble is wrong. The learning rate (shrinkage) scales each tree\'s contribution.' },
      { id: 'q2', prompt: 'scale_pos_weight in XGBoost for imbalanced fraud detection (1% fraud) should be set to approximately:', options: ['1.0', '0.01', '99 (ratio of negatives to positives)', '0.5'], answerIndex: 2, explanation: 'scale_pos_weight = sum(negative_cases) / sum(positive_cases). For 1% fraud: 99/1 = 99. This tells XGBoost to weight each fraud case 99× more heavily, compensating for class imbalance.' },
      { id: 'q3', prompt: 'LightGBM\'s key speed advantage over XGBoost comes from:', options: ['Using fewer trees', 'Leaf-wise tree growth and histogram-based splits — reduces memory and computation vs XGBoost\'s level-wise growth', 'Running on GPU by default', 'Using smaller trees with depth=1'], answerIndex: 1, explanation: 'LightGBM bins continuous features into histograms (fast split finding) and grows the leaf with maximum loss reduction (vs growing all leaves at same depth). This makes it 10-100× faster on large datasets.' },
      { id: 'q4', prompt: 'In a stacking ensemble, the meta-learner is trained on:', options: ['The original training features', 'Out-of-fold predictions from base models — learning how to best combine base model outputs on unseen data', 'The test set predictions', 'The average of all base model outputs'], answerIndex: 1, explanation: 'Using k-fold cross-validation, base models predict on the held-out fold they weren\'t trained on. These out-of-fold predictions are the meta-features. The meta-learner learns to weight base models appropriately.' },
      { id: 'q5', prompt: 'SHAP (SHapley Additive exPlanations) values for a fraud model tell you:', options: ['The model\'s overall AUC', 'For each prediction, how much each feature contributed to moving the score from the baseline — enabling per-transaction explanations required for compliance', 'Which features to remove', 'The training speed of the model'], answerIndex: 1, explanation: 'SHAP is the gold standard for ML explainability. Based on game theory (Shapley values), it fairly distributes the prediction\'s deviation from baseline among all features — both globally and for individual predictions.' }
    ]
  },
  'ml-a-6': {
    title: 'Check your understanding: Reinforcement Learning',
    intro: 'Test your knowledge of RL fundamentals, Q-learning, and bandits.',
    questions: [
      { id: 'q1', prompt: 'The key difference between RL and supervised learning is:', options: ['RL uses more data', 'RL learns from rewards through environment interaction — no labelled training examples. The agent must discover which actions lead to high cumulative reward by trial and error.', 'RL only works for games', 'Supervised learning cannot handle sequences'], answerIndex: 1, explanation: 'Supervised: learn f(x)→y from labelled pairs. RL: learn a policy π(s)→a by maximising cumulative reward through interaction. No teacher provides the "correct" action — only a reward signal after the fact.' },
      { id: 'q2', prompt: 'The Q-function Q(s,a) represents:', options: ['The probability of taking action a in state s', 'The expected cumulative discounted reward of taking action a in state s and following the optimal policy thereafter', 'The immediate reward of action a', 'The state transition probability'], answerIndex: 1, explanation: 'Q-values capture long-term value, not just immediate reward. Q*(s,a) is the expected return from (s,a) under the optimal policy. The Bellman equation: Q(s,a) = r + γ × max_a\'[Q(s\',a\')].' },
      { id: 'q3', prompt: 'Epsilon-greedy exploration with ε=0.1 means the agent:', options: ['Explores 90% of the time', 'Explores randomly 10% of the time, exploits the best-known action 90% of the time', 'Decays epsilon by 0.1 each episode', 'Sets the learning rate to 0.1'], answerIndex: 1, explanation: 'Epsilon-greedy balances exploration (trying new actions to gather information) vs exploitation (using the best known action). High epsilon early, decay toward 0.05 as the agent learns.' },
      { id: 'q4', prompt: 'Experience replay in DQN helps because:', options: ['It reuses data, reducing the need for environment interaction', 'Storing (s,a,r,s\') transitions and sampling random mini-batches breaks correlations between consecutive transitions — making training more stable and data-efficient', 'It doubles training speed', 'It removes the need for a target network'], answerIndex: 1, explanation: 'Without replay, consecutive transitions are highly correlated (s,s+1,s+2...). Training on correlated data causes instability. Random sampling from the replay buffer decorrelates transitions, stabilising learning.' },
      { id: 'q5', prompt: 'Thompson Sampling for multi-armed bandits outperforms epsilon-greedy because:', options: ['It requires less computation', 'It maintains a probability distribution over each arm\'s reward rate and samples from it — naturally balancing exploration and exploitation based on uncertainty', 'It always explores more', 'It does not need prior knowledge'], answerIndex: 1, explanation: 'Thompson Sampling explores arms with high uncertainty (wide posteriors) and exploits known good arms (narrow posteriors around high values). This uncertainty-aware exploration is more efficient than the fixed epsilon in ε-greedy.' }
    ]
  },
